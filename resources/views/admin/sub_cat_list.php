...
if(isset($_POST['filter']))
{
	...
	{
		 $sess_where .= " and parent_id = '".$_REQUEST['parent_id']."' ";
		 $_SESSION['parent_id_session'] = trim($_REQUEST['parent_id']);
	}
	else
	{
		unset($_SESSION['parent_id_session']);
	}
...
if(isset($status) && !empty($status))
{
	...
	$db->query($sqlquery);
	header("Location:sub_cat_list.php?msg=$update_ok_msg&case=1");
	exit;
}
?>
<html>
...
<table style="border-collapse: collapse;" border="0" cellpadding="0" width="100%" height="100%">
                                    ...
                                    style="border:1px solid #000000; padding:10px;">
                                        <tbody>
                                            ...
                                            <tr height="30">
                                                ...
                                                <td align="center">
                                                    <select name="parent_id" id="parent_id" class="Field300">
													...
													if($sql_arr)
													{
														foreach($sql_arr as $val)
														{
															...
															$cat_name = stripcslashes(html_entity_decode($val['cat_name']));	
																if($_SESSION['parent_id_session']==$cat_id)
																{
                                ...
                                <table cellpadding="0" cellspacing="0" class="Panel">
                                    <tbody>
                                      ...
                                      <tr>
                                        ...
                                        <a href="sub_cat_list.php?sortby=cat_name_asc&page=<?php echo $page;?>" class="link_class">Category Name</a>
                                        <?php }else{?>
                                        <a href="sub_cat_list.php?sortby=cat_name_desc&page=<?php echo $page;?>" class="link_class">Category Name</a>
                                        <?php }?>
                                        </td>
                                        <td width="150" id="Heading_list">Level1 Category</td>
                                        <td width="150" id="Heading_list">Level2 Category</td>
										<td width="150" id="Heading_list">Level3 Category</td>
										<td width="150" id="Heading_list">Level4 Category</td>
                                        <td width="70" id="Heading_list">
                                        <?php if($sortby == 'statusdesc'){?>
                                        <a href="sub_cat_list.php?sortby=statusasc&page=<?php echo $page;?>" class="link_class">Status</a>
                                        <?php }else{?>
                                        <a href="sub_cat_list.php?sortby=statusdesc&page=<?php echo $page;?>" class="link_class">Status</a>
                                        <?php }?>
                                        </td>
									  ...
									  <form action="<?php echo SERVER_ADMIN_PATH;?>process/sub_cat_actions.php" method="post" id="sub_cat_frm">
									  <?php
											
											//============================================================
											//PAGGING CODE STARTS HERE
											$qry_count_mypro = "SELECT cat_id FROM tbl_categories where 
											 level='5' and 1=1 $session_where  $orderby";
											$res_count_mypro = mysqli_query($db->dbh, $qry_count_mypro);
												
											$targetpage = "sub_cat_list.php"; //your file name  (the name of this file)
										...
										$cat_list="select * from tbl_categories where 1=1 and level='5'
									  ...
									  <tr bgcolor="<?php echo $bgcolor; ?>" onMouseOver="changebackcolor_hover('row<?php echo $cat_id;?>')" onMouseOut="changebackcolor_blur('row<?php echo $cat_id;?>')" id="row<?php echo $cat_id;?>">
										...
										if($status==1)
										{
										  ...
										  <?php
											if($status==0)
											{
												echo '<a href="sub_cat_list.php?status='.base64_encode(1).'&status_id='.base64_encode($cat_id).'"><img src="images/disable.gif" border="0" class="Action" title="Activate"></a>'; 
											}
											if($status==1)
											{
												echo '<a href="sub_cat_list.php?status='.base64_encode(0).'&status_id='.base64_encode($cat_id).'"><img src="images/enable.gif" border="0" class="Action" title="Blocked"></a>'; 
											}
										  ?></td>
    ...
    &nbsp;&nbsp; <input type="checkbox" class="check-all" name="cat_ids[]" id="cat_ids[]" value="<?php echo base64_encode($cat_id);?>" style="margin-top:-5px;" />
										...
										{
										?>	
                                        <a href="edit_sub_cat.php?edit_id=<?php echo base64_encode($cat_id);?>"><img src="images/edit.gif" border="0" title="Edit" class="Action"></a>
                                        <?php
										}
										?>
										<?php
                                      ...
                                      <tr bgcolor="<?php echo $bgcolor; ?>" onMouseOver="changebackcolor_hover('row<?php echo $cat_id;?>')" onMouseOut="changebackcolor_blur('row<?php echo $cat_id;?>')" id="row<?php echo $cat_id;?>">
                                        <td nowrap="nowrap" class="SmallFieldLabel" width="30">&nbsp;</td>
                                        <td nowrap="nowrap" class="SmallFieldLabe2" colspan="6">
                                         <?php
										if($top_reviews_module=='Yes')
										{
										?>
											<a href="<?php echo SERVER_ADMIN_PATH;?>reviews_list_category.php?key=<?php echo base64_encode($cat_id);?>" class="cat_href"> View Reviews</a> |
                                            <a href="<?php echo SERVER_ADMIN_PATH;?>gcomments.php?key=<?php echo base64_encode($cat_id);?>" class="cat_href"> View Discussions</a> |
                                            
                                            <a href="<?php echo SERVER_ADMIN_PATH;?>category_comments.php?key=<?php echo base64_encode($cat_id);?>" class="cat_href"> View Comments</a> |
                                            
                                            <a href="<?php echo SERVER_ADMIN_PATH;?>questions_list.php?key=<?php echo base64_encode($cat_id);?>" class="cat_href"> View Questions</a> |
                                            <a href="<?php echo SERVER_ADMIN_PATH;?>answers_list.php?key=<?php echo base64_encode($cat_id);?>" class="cat_href"> View Answers</a>  |
                                            
                                            <?php
											if($is_featured_topic=='No' && $status==1)
											{
											?>
                                           	 <a href="javascript:;" onClick="set_featured_topic('<?php echo base64_encode($cat_id);?>')" class="cat_href">Set as Featured</a>
                                            <?php
											}
											elseif($is_featured_topic=='Yes' )
											{
											?>
                                           	 <a href="javascript:;" onClick="unset_featured_topic('<?php echo base64_encode($cat_id);?>')" class="cat_href">UnSet as Featured</a>
                                            <?php
											}
											?> 
										<?php
                                        ...
                                        <td nowrap="nowrap" class="SmallFieldLabel righttd_border" width="97">&nbsp;</td> 
                                        </tr>
                                      <?php
											}
										}
