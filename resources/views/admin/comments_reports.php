...
if(isset($_POST['filter']))
{
	...
	if($_REQUEST['c_report_deatil']!="")
	{
		 $sess_where .= " and c_report_deatil  like \"%".trim($_REQUEST['c_report_deatil'])."%\" ";
		 $_SESSION['c_report_user_id_sess'] = trim($_REQUEST['c_report_deatil']);
	}
	else
	{
		unset($_SESSION['c_report_deatil_sess']);
	}
	
	if($_REQUEST['c_report_user_id'] != "")
	{
		$sess_where .= " and c_report_user_id = '".$_REQUEST['c_report_user_id']."'";
		$_SESSION['c_report_user_id_sess'] = $_REQUEST['c_report_user_id'];
	}
	else
	{
		unset($_SESSION['c_report_user_id_sess']);
	}
...
{
	unset($_SESSION['c_report_deatil_sess']);
	$_SESSION['c_report_deatil_sess']="";
	
	unset($_SESSION['c_report_user_id_sess']);
	$_SESSION['c_report_user_id_sess']="";

	unset($_SESSION['sess_rreport']);
	$_SESSION['sess_rreport']="";
	
	header("Location:comments_reports.php?key=".$_REQUEST['key']."&review=".$_REQUEST['review']);
}
...
{
	case "user_desc":
		$orderby	= " ORDER BY c_report_user_id desc";
	break;
	
	case "user_asc":
		$orderby	= " ORDER BY c_report_user_id asc";
	break;
	...
	default:
		$orderby = "ORDER BY c_report_id desc";
	break;
}
...
<table style="border-collapse: collapse;" border="0" cellpadding="0" width="100%" height="100%">
                      ...
                      <tr>
                        <td class="body"><table id="Table1" border="0" cellpadding="0" cellspacing="0" width="100%">
                              <tr>
								...
								if($review!='')
								{
									$comment_path = SERVER_ADMIN_PATH.'review_comments.php?key='.$review;
								}
								else
								{
									$comment_path = SERVER_ADMIN_PATH.'comments_list.php';
								}
								?>
                                <td><a href="<?php echo SERVER_ADMIN_PATH;?>index.php">Home</a> &raquo; <a>&raquo; <a href="<?php echo $comment_path;?>">Comments Listing</a>&raquo; <a>Comments Report Listing</a></td>
                              </tr>
							  <?php
                                    ...
                                    style="border:1px solid #000000; padding:10px;">
                                        <tbody>
                                            ...
                                            <tr height="30">
                                                ...
                                                <td align="center">
                                                <input name="c_report_deatil" id="c_report_deatil" class="Field300" 
                                                value="<?php echo $_SESSION['c_report_deatil_sess'];?>" type="text"/>
                                                </td>
                                            </tr>
                                            <tr height="30">
                                                <td class="SmallFieldLabelnew font_bold" align="left"  width="150"> 
                                                ...
                                                <td align="center">
                                               	<select name="c_report_user_id" id="c_report_user_id" 
                                                   ...
                                                   <option value=""> ------ Please Select User ------</option>
                                                     ...
                                                     if($users_arr)
                                                     {
                                                        foreach($users_arr as $val)
                                                        {
                                                            ...
                                                            if($_SESSION['c_report_user_id_sess']==$user_id)
                                                            {
                                ...
                                <table cellpadding="0" cellspacing="0" class="Panel">
                                    <tbody>
                                      ...
                                      <tr>
                                        ...
                                        <a href="comments_reports.php?sortby=user_asc&page=<?php echo $page;?>&key=<?php echo $key;?>" class="link_class">Report User</a>
                                        <?php }else{?>
                                        <a href="comments_reports.php?sortby=user_desc&page=<?php echo $page;?>&key=<?php echo $key;?>" class="link_class">Report User</a>
                                        <?php }?>
                                        </td>
									  ...
									  <form action="<?php echo SERVER_ADMIN_PATH; ?>process/comment_report_actions.php" method="post" id="c_report_form">
									  <?php
									  		
											//============================================================
											//PAGGING CODE STARTS HERE
											$qry_count_mypro = "SELECT c_report_id FROM tbl_comment_report where 1=1
											and c_report_comment_id='".$commentsid."' $session_where $orderby";
											$res_count_mypro = mysqli_query($db->dbh, $qry_count_mypro);
												
											$targetpage = "comments_reports.php"; 
										...
										$report_query="select * from tbl_comment_report where 1=1 and 
										c_report_comment_id='".$commentsid."' $session_where $orderby 
										LIMIT $start, $limit";	
										...
										if(isset($report_arr))
										{
											...
											{
												$c_report_id	  = $val['c_report_id'];	
												$c_report_user_id = $val['c_report_user_id'];
												$c_report_deatil = $val['c_report_deatil'];
												$c_report_status   = $val['c_report_status'];
												$c_report_date    = $val['c_report_date'];
												$details  = stripslashes(html_entity_decode($c_report_deatil));
												$details   = wordwrap($details,100," ",true);
												$r_report_option  = $val['r_report_option'];
												
												$select_qry ="select user_name from tbl_users where 
												user_id='".$c_report_user_id."' ";
                                                $select_ar  = $db->get_row($select_qry,ARRAY_A);
												$user_name = stripslashes(html_entity_decode($select_ar['user_name']));
												$user_name = wordwrap($user_name,100," ",true);
									  ...
									  <tr bgcolor="<?php echo $bgcolor; ?>" onMouseOver="changebackcolor_hover('row<?php echo $c_report_id;?>')" onMouseOut="changebackcolor_blur('row<?php echo $c_report_id;?>')" id="row<?php echo $c_report_id;?>">
                                        ...
                                        <td nowrap="nowrap" class="SmallFieldLabel" width="300">
                                            <div id="before_details_div_<?php echo $c_report_id;?>"> 
                                            <?php
											...
											{
												echo '<a href="javascript:;" onClick="show_detail('.$c_report_id.');" style="text-decoration:none;">'.substr($details,0,100).'</a>&nbsp';
												
												echo '<a href="javascript:;" onClick="show_detail('.$c_report_id.');" style="color:#0000FF;"> <strong>&raquo; More</strong></a>';
											}
											?>
                                            </div>
                                            <div id="after_details_div_<?php echo $c_report_id;?>" style="display:none;"> 
                                            	<?php
													echo '<a href="javascript:;" onClick="show_detail('.$c_report_id.');" style="text-decoration:none;">'.$details.'</a>&nbsp;';
													
													echo '<a href="javascript:;" onClick="show_detail('.$c_report_id.');" style="color:#0000FF;"><strong>&raquo; Close </strong></a>';
												?>
                                            </div>
                                        </td>
    ...
    &nbsp;&nbsp; <input type="checkbox" class="check-all" name="c_report_ids[]" id="c_report_ids[]" value="<?php echo base64_encode($c_report_id);?>" style="margin-top:-5px;" />
    								
										&nbsp; &nbsp;
                                        <a href="javascript:;" onClick="delete_comment_report('<?php echo $c_report_id;?>')"><img src="images/delet.gif" border="0" title="Delete Report" class="Action" ></a>
                                        </td>
                                        <?php
										}
										?>
									...
									?>
									  ...
									  <tr>
                                        ...
                                        <span style="float:right; padding-bottom:10px; margin-right:8px;">
                                            <select name="dropdown" onChange="multiple_action('c_report_form');">
