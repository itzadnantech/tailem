	...
	?>
        <table class="process_table">
             <tr>
                ...
                <td width="88%">
                    <select name="level3_cat_id" id="level3_cat_id" style="width:300px;padding:4px 1px;" onChange="category_level4_review(this.value,'<?php echo $reviews_id;?>')">
                        ...
                        if($sql_arr)
                        {
                            foreach($sql_arr as $val)
                            {
								...
								$level3_qry = "select cat_level3 from tbl_reviews where review_id='".$reviews_id."'";
								$level3_arr = $db->get_row($level3_qry,ARRAY_A);
								$selected_level3_id  = $level3_arr['cat_level3'];
