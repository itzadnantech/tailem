...

{
...

	{
	 	...
	 	$checkbox = $_POST['video_ids']; //from name="checkbox[]"

	 	$countCheck = count($_POST['video_ids']);
...

		{
			...
			$sql = "DELETE from tbl_videos where video_id = '".$del_id."'";
		...
		{		
			 ...
			 header('Location:'.SERVER_ADMIN_PATH."video_list.php?msg=$okmsg&case=1");
...

	{
...

		 {
			 ...
			 $qry = "select status from tbl_videos where video_id='".$del_id."'";
		     ...
		     $sql = "update tbl_videos set status=$status where video_id='".$del_id."'";
...

{
	...
	header('Location:'.SERVER_ADMIN_PATH."video_list.php?msg=$errormsg&case=2");
