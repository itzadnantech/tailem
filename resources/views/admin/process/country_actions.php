...
if(!empty($_POST['country_ids']))
{
	...
	{
	 	$checkbox = $_POST['country_ids']; //from name="checkbox[]"
	 	$countCheck = count($_POST['country_ids']);
	
	 	for($i=0;$i<$countCheck;$i++)
		{
			$del_id     = base64_decode($checkbox[$i]);
			$sql = "DELETE from tbl_countries where country_id = '".$del_id."'";
		...
		{		
			 ...
			 header('Location:'.SERVER_ADMIN_PATH."countries_listing.php?msg=$okmsg&case=1");
		}
		else
		{ 
	...
	if($_POST['dropdown']=='Active') // from button name="delete"
	{
		 ...
		 {
			 $del_id  = base64_decode($checkbox[$i]);
			 $qry = "select status from tbl_countries where country_id='".$del_id."'";
			 ...
			 if($status==0)
			 {
		 	 ...
		 	 $del_id  = base64_decode($checkbox[$i]);
		     $sql = "update tbl_countries set status=$status where country_id='".$del_id."'";
...
else
{
	...
	header('Location:'.SERVER_ADMIN_PATH."countries_listing.php?msg=$errormsg&case=2");
}
?>