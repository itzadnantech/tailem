...
if(!empty($_POST['ads_ids']))
{
 	if($_POST['dropdown']=='Delete') // from button name="delete"
	{
	 	$checkbox = $_POST['ads_ids']; //from name="checkbox[]"
	 	$countCheck = count($_POST['ads_ids']);
	
	 	for($i=0;$i<$countCheck;$i++)
		{
			$del_id     = base64_decode($checkbox[$i]);
			$sql = "DELETE from tbl_mobile_advertisement where ads_id = '".$del_id."'";
		...
		{		
			 ...
			 header('Location:'.SERVER_ADMIN_PATH."ads_mobile_list.php?msg=$okmsg&case=1");
		}
		else
		{ 
	...
	if($_POST['dropdown']=='Active') // from button name="delete"
	{
		 ...
		 {
			 $del_id  = base64_decode($checkbox[$i]);
			 $qry = "select status from tbl_mobile_advertisement where ads_id='".$del_id."'";
			 ...
			 if($status==0)
			 {
		 	 ...
		 	 $del_id  = base64_decode($checkbox[$i]);
		     $sql = "update tbl_mobile_advertisement set status=$status where ads_id='".$del_id."'";
...
else
{
	...
	header('Location:'.SERVER_ADMIN_PATH."ads_mobile_list.php?msg=$errormsg&case=2");
}
?>