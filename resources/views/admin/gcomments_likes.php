...
if(isset($_POST['filter']))
{
	...
	if($_REQUEST['g_comment_like_user_id'] != "")
	{
		$sess_where .= " and g_comment_like_user_id = '".$_REQUEST['g_comment_like_user_id']."'";
		$_SESSION['g_comment_like_user_id_sess'] = $_REQUEST['g_comment_like_user_id'];
	}
	else
	{
		unset($_SESSION['g_comment_like_user_id_sess']);
	}
...
{
	unset($_SESSION['g_comment_like_user_id_sess']);
	$_SESSION['g_comment_like_user_id_sess']="";

	unset($_SESSION['sess_comment_likes']);
	$_SESSION['sess_comment_likes']="";
	
	header("Location:gcomments_likes.php?key=".$_REQUEST['key']."&review=".$_REQUEST['review']);
}
...
{
	case "user_desc":
		$orderby	= " ORDER BY g_comment_like_user_id desc";
	break;
	
	case "user_asc":
		$orderby	= " ORDER BY g_comment_like_user_id asc";
	break;
	
	case "date_desc":
		$orderby	= " ORDER BY g_comment_like_date desc";
	break;
	
	case "date_asc":
		$orderby	= " ORDER BY g_comment_like_date asc";
	break;
	
	default:
		$orderby = "ORDER BY g_comment_like_id desc";
	break;
}
...
<table style="border-collapse: collapse;" border="0" cellpadding="0" width="100%" height="100%">
                      ...
                      <tr>
                        <td class="body"><table id="Table1" border="0" cellpadding="0" cellspacing="0" width="100%">
                              <tr>
								...
								?>
                                <a href="<?php echo SERVER_ADMIN_PATH;?>index.php">Home</a> &raquo; <a href="<?php echo $comment_path;?>">Comments Listing</a>&raquo; <a>Comments Report Listing</a></td>
                              </tr>
							  <?php
                              $comments_qry="select gcomment_user_id,gcomment_cat_id,gcomment_detail, 
							  gcomment_post_date, gcomment_status, gcomment_ip from tbl_general_comments where 
							  gcomment_id='".$gcommentsid."' ";
                                    ...
                                    style="border:1px solid #000000; padding:10px;">
                                        <tbody>
                                            ...
                                            <tr height="30">
                                                ...
                                                <td align="center">
                                               	<select name="g_comment_like_user_id" id="g_comment_like_user_id" 
                                                   ...
                                                   <option value=""> ------ Please Select User ------</option>
                                                     ...
                                                     if($users_arr)
                                                     {
                                                        foreach($users_arr as $val)
                                                        {
                                                            ...
                                                            if($_SESSION['g_comment_like_user_id_sess']==$user_id)
                                                            {
                              ...
                              <tr>
                                ...
                                <?php
									...
									$gcomment_detail=stripslashes(html_entity_decode($comment_arr['gcomment_detail']));
									$gcomment_user_id = $comment_arr['gcomment_user_id'];
                                ...
                                <table cellpadding="0" cellspacing="0" class="Panel">
                                    <tbody>
                                      ...
                                      <tr>
                                        ...
                                        <a href="gcomments_likes.php?sortby=user_asc&page=<?php echo $page;?>&key=<?php echo $key;?>" class="link_class">Report User</a>
                                        <?php }else{?>
                                        <a href="gcomments_likes.php?sortby=user_desc&page=<?php echo $page;?>&key=<?php echo $key;?>" class="link_class">Report User</a>
                                        <?php }?>
                                        </td>
                                        <td width="100" id="Heading_list">
                                        <?php if($sortby == 'date_desc'){?>
                                        <a href="gcomments_likes.php?sortby=date_asc&page=<?php echo $page;?>&key=<?php echo $key;?>" class="link_class">Like Date</a>
                                        <?php }else{?>
                                        <a href="gcomments_likes.php?sortby=date_desc&page=<?php echo $page;?>&key=<?php echo $key;?>" class="link_class">Like Date</a>
                                        <?php }?>
                                        </td>
                                        <?php
									  ...
									  <?php
											...
											$qry_count_mypro="SELECT g_comment_like_id FROM tbl_general_comments_likes 
											where 1=1 and g_comment_id='".$gcommentsid."' $session_where $orderby";
											$res_count_mypro = mysqli_query($db->dbh, $qry_count_mypro);
												
											$targetpage = "gcomments_likes.php"; 
										...
										$report_query="select * from tbl_general_comments_likes where 1=1 and 
										g_comment_id='".$gcommentsid."' $session_where $orderby 
										LIMIT $start, $limit";	
										...
										if(isset($report_arr))
										{
											...
											{
												$g_comment_like_id	  = $val['g_comment_like_id'];	
												$g_comment_like_user_id = $val['g_comment_like_user_id'];
												$g_comment_like_status  = $val['g_comment_like_status'];
												$g_comment_like_date    = $val['g_comment_like_date'];
												
												$select_qry ="select user_name from tbl_users where 
												user_id='".$g_comment_like_user_id."' ";
                                                $select_ar  = $db->get_row($select_qry,ARRAY_A);
												$user_name = stripslashes(html_entity_decode($select_ar['user_name']));
												$user_name = wordwrap($user_name,100," ",true);
									  ...
									  <tr bgcolor="<?php echo $bgcolor; ?>" onMouseOver="changebackcolor_hover('row<?php echo $g_comment_like_id;?>')" onMouseOut="changebackcolor_blur('row<?php echo $g_comment_like_id;?>')" id="row<?php echo $g_comment_like_id;?>">
                                        <td nowrap="nowrap" class="SmallFieldLabel" width="30"><?php echo $sr_no;?></td>
                                        <td nowrap="nowrap" class="SmallFieldLabel" width="200">
											<?php echo $user_name;?>
                                        </td>
                                        <td nowrap="nowrap" class="SmallFieldLabel" width="200">
											<?php echo date("d M Y",$g_comment_like_date);?>
                                        </td>
    ...
    &nbsp;&nbsp; <input type="checkbox" class="check-all" name="g_comment_like_ids[]" id="g_comment_like_ids[]" value="<?php echo base64_encode($g_comment_like_id);?>" style="margin-top:-5px;" />
    								
										&nbsp; &nbsp;
                                        <a href="javascript:;" onClick="delete_comment_like('<?php echo $g_comment_like_id;?>')"><img src="images/delet.gif" border="0" title="Delete Report" class="Action" ></a>
                                        </td>
                                        <?php
										 }
										 ?>
